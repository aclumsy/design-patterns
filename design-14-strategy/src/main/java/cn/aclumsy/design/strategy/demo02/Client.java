...
public class Client {
    ...
    public static void main(String[] args) {
        List<Receipt> receipts = ReceiptBuilder.buildReceipts();
        for (Receipt receipt : receipts) {
            if ("MT1101".equals(receipt.getType())) {
                System.out.println("处理MT1101回执信息");
            } else if ("MT1102".equals(receipt.getType())) {
                System.out.println("处理MT1102回执信息");
            } else if ("MT1103".equals(receipt.getType())) {
                System.out.println("处理MT1103回执信息");
            } else if ("MT1104".equals(receipt.getType())) {
                System.out.println("处理MT1104回执信息");
            }
        }
    }
}
